...
export default function StudentUploadPhoto() {
  const [projectContainer, setProjectContainer] = useState([]);

  useEffect(() => {
      const storedProjects = JSON.parse(localStorage.getItem("CinemaPhotosStudents"));
      if (storedProjects) {
  ...
  const deleteAll = () => {
      localStorage.removeItem("CinemaPhotosStudents");
      setProjectContainer([]);
  };
